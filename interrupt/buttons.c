...
#define PORTC_D_IRQ_NBR (IRQn_Type) -1					/* ToDo 5.1: Define interrupt (IRQ) number for PORTC_PORTD. See interrupt vector assignments in NVIC section of KL46 Sub-Family Reference Manual - NVIC section*/ 
 ...
 *----------------------------------------------------------------------------*/
void put_PORTC_PORTD_irq_handler_name(void){		/* ToDo 5.2: Put a proper name of PORTC_PORTD Interrupt service routine ISR. See system_MKL46Z4.s file for function name */ 
	
	buttonsService();

	//while((FPTC->PDIR&(1<<SW1_PIN))==0);				/* ToDo 7.1: Enable wait in the interrupt for SW1 button release */
	
	//PORTC->PCR[SW1_PIN] |= PORT_PCR_X_MASK; 		/* ToDo 5.3: Clear interrupt service flag in port control register otherwise int. remains active */
}
...
void buttonsInitialize(void){
	...
	//PORTC->PCR[SW1_PIN] |=  PORT_PCR_XXX_MASK |		/* ToDo 5.4: Set bit in PCR register to enable pull resistor. See KL46 Sub-Family Reference Manual */
	//												PORT_PCR_XXX_MASK;		/* ToDo 5.5: Set bit in PCR register to select pull up. See KL46 Sub-Family Reference Manual */
	//PORTC->PCR[SW1_PIN] |= 	PORT_PCR_XXX(xxx);		/* ToDo 5.6: Set value for IRQC bit field in PCR register to select falling edge interrupts for PORTC[3]. See KL46 Sub-Family Reference Manual */
